...
@section('breadcrumb', 'Soal / Edit')

@section('content')
<div style="max-width:680px;">
    <div class="page-header">
        <div class="page-title">Edit Soal</div>
        <div class="page-sub">Perbarui soal #{{ $question->id }}</div>
    </div>
    ...
    <div class="a-card">
        <div class="a-card-body">
            <form method="POST" action="{{ route('admin.questions.update', $question) }}" id="questionForm">
                ...
                <div class="a-form-group">
                    <label class="a-label">Instruksi</label>
                    <input type="text" name="instruction" class="a-input"
                           value="{{ old('instruction', $question->instruction) }}"/>
                </div>

                <div class="a-form-group">
                    <label class="a-label">Teks Soal <span style="color:#ba1a1a">*</span></label>
                    <textarea name="question_text" class="a-textarea" required>{{ old('question_text', $question->question_text) }}</textarea>
                ...
                <div class="a-form-group">
                    ...
                    <select name="question_type" id="questionType" class="a-select" required>
                        ...
                        <option value="fill_blank" {{ old('question_type', $question->question_type) === 'fill_blank' ? 'selected' : '' }}>Isian Singkat</option>
                    </select>
                </div>

                {{-- Options --}}
                <div id="optionsSection" style="{{ $question->question_type !== 'multiple_choice' ? 'display:none' : '' }}">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.6rem;">
                        <label class="a-label" style="margin-bottom:0;">Pilihan Jawaban</label>
...
function createOptionRow(index, text = '', isCorrect = false) {
    ...
    row.innerHTML = `
        <input type="checkbox" name="options[${index}][is_correct]" value="${isCorrect ? 1 : 0}"
               ${isCorrect ? 'checked' : ''}
               style="width:1rem;height:1rem;accent-color:#6b3f00;flex-shrink:0;">
        <input type="text" name="options[${index}][text]" value="${text.replace(/"/g,'&quot;')}" required
    ...
    cb.addEventListener('change', () => { cb.value = cb.checked ? 1 : 0; });
    return row;
}
...
existingOptions.forEach(opt => {
    document.getElementById('optionsList').appendChild(
        createOptionRow(optionCount++, opt.option_text, !!opt.is_correct)
    );
});
</script>
@endpush
